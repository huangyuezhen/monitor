...
class RabbitMQAPI(object):
    ...
    def list_queues(self, filters=None ,dlq=False):
        '''
            ...
            for _filter in filters:
                ...
                if len(shared_items) == len(_filter):
                    if not dlq:
                        ...
                        if queue['name'] != "aliveness-test" and not re.match(r'^dlq',queue['name']):
                            queues.append(element)
                    else:
			element = {'{#NODENAME}': queue['node'],
                                   '{#VHOSTNAME}': queue['vhost'],
                                   '{#DLQNAME}': queue['name']}
                        if re.match(r'^dlq',queue['name']):
                            queues.append(element)
    ...
    def check_node(self, item):
       '''Return the value for a specific item about node info'''
       nodes_info = self.call_api('nodes')
       nodes_type = []
       nodes_alive = []
       for node in nodes_info:
	   if node['running']:
               nodes_type.append(node['type'])
               nodes_alive.append(node['name'])
       if item == "count":
           return len(nodes_alive) if nodes_alive else 0
       elif item == "disc_count":
           return nodes_type.count('disc') if nodes_type else 0
       elif item == "ram_count":
           return nodes_type.count('ram') if nodes_type else 0
        ...
        for queue in self.call_api('queues'):
            ...
            for _filter in filters:
                ...
                if len(shared_items) == len(_filter):
                    success = True
                    break
            if success and queue['name'] != "aliveness-test":
...
def main():
    ...
    choices = ['list_queues', 'list_dlq_queues' ,'list_shovels', 'list_nodes', 'queues', 'shovels', 'check_aliveness',
               'node','server']
    ...
    elif options.check == 'list_dlq_queues':
	print json.dumps({'data': api.list_queues(filters,dlq=True)})
    ...
    elif options.check == 'node':
        if not options.metric:
            parser.error('Missing required parameter: "metric"')
        else:
            print api.check_node(options.metric)
